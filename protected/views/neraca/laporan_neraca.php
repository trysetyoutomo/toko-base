...
(
    CASE 
				...
				WHEN sum( debit ) > sum( kredit ) > 0 AND ( ag.id = 1 OR ag.id = 5) THEN sum( debit ) - sum( kredit )
				ELSE 0
    END) AS debit,
		(
		CASE 
				WHEN sum( kredit ) > sum( debit ) > 0 AND ( ag.id = 2 OR ag.id = 3 OR ag.id = 4 ) THEN sum( kredit ) - sum( debit )
				WHEN sum( kredit ) > sum( debit ) > 0 AND ( ag.id = 1 OR ag.id = 5) THEN sum( kredit ) - sum( debit )
				else 0
