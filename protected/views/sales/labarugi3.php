...
function getAccounts($modifiedData,$filter,$totalNitip){
	...
	if (isset($PENDAPATAN) && isset($HPP)){
		
		$html .= '
		<tr>
		<td  style="text-align: left;padding-left:1rem;font-weight:bolder">LABA KOTOR</td>
		<td style="text-align: right;">'.number_format($PENDAPATAN - $HPP).'</td>
		</tr>'
		;
		$totalNitip = $PENDAPATAN - $HPP;
	}
...
$totalPendapatanPokok = 0;
$totalBiaya = 0;
$table1 = getAccounts($modifiedData,['PENDAPATAN','HPP'],$totalPendapatanPokok);
echo $table1['html'];
$table2 = getAccounts($modifiedData,['BIAYA'],$totalBiaya);
echo $table2['html'];
echo '
<tr>
