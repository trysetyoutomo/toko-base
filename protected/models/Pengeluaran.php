...
class Pengeluaran extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('tanggal, jenis_pengeluaran, user, keterangan, total', 'required'),
			array('total', 'numerical', 'integerOnly'=>true),
			array('jenis_pengeluaran, user', 'length', 'max'=>60),
			array('keterangan', 'length', 'max'=>100),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, tanggal, jenis_pengeluaran, user, keterangan, total', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('jenis_pengeluaran',$this->jenis_pengeluaran,true);
